"""
fine_tune_transformer2.py
=========================
Fine‑tunes a BERT model for 3‑class sentiment classification
on restaurant reviews.

Data Transformation (WordPiece tokenisation via HuggingFace):
    • input_ids        – WordPiece token indices
    • attention_mask   – 1 for real tokens, 0 for padding
    • token_type_ids   – segment IDs (BERT uses these natively)

Model  : bert-base-uncased  →  BertForSequenceClassification (3 classes)
...
print()
print("[NOTE] BERT natively uses token_type_ids (segment IDs) to distinguish "
      "sentence pairs. For single-sentence classification they are all zeros, "
      "but they are still passed to the model's forward pass.")
print()
...
# BERT accepts token_type_ids (unlike DistilBERT), so we keep all three keys:
#   input_ids, attention_mask, token_type_ids
...
if epoch_logs:
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(PLOT_DIR, "bert_training_curves.png"), dpi=150)
    plt.show()
    print(f"[INFO] Training curves saved to {PLOT_DIR}/bert_training_curves.png\n")
...
ax.set_title("Confusion Matrix – BERT", fontsize=14, fontweight="bold")
plt.tight_layout()
fig.savefig(os.path.join(PLOT_DIR, "cm_bert.png"), dpi=150)
plt.show()
print(f"[INFO] Confusion matrix saved to {PLOT_DIR}/cm_bert.png\n")
...
# Prepare a single sample (keep batch dimension) – include token_type_ids for BERT
single_input = {
    "input_ids":       test_encodings["input_ids"][0:1].to(DEVICE),
    "attention_mask":  test_encodings["attention_mask"][0:1].to(DEVICE),
    "token_type_ids":  test_encodings["token_type_ids"][0:1].to(DEVICE),
}
...
tracker = EmissionsTracker(
    project_name="single_predict_BERT",
...
print(f"\n[BERT]  Energy: {energy_kwh:.10f} kWh  |  CO₂: {emissions:.10f} kg")
